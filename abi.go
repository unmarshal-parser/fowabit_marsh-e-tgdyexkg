...
func (_Main *MainFilterer) ParseABITransferEvent(log types.Log, parsedLog ethrpc.Log) (*ABITransferEvent, error) {
	e := new(ABITransferEvent)
	if err := _Main.contract.UnpackLog(e, "Transfer", log); err != nil {
		return nil, err
	}
	e.Raw = parsedLog
	return e, nil
...
type ABIApproveMethod struct {
	Spender        common.Address
	Amount         *big.Int
	RawTransaction sdkTransactionTypes.RawTransaction
}

func (_Main *MainFilterer) ParseABIApproveMethod(parsedTx sdkTransactionTypes.RawTransaction) (*ABIApproveMethod, error) {
	...
	m := new(ABIApproveMethod)
	if err := _Main.UnpackMethodIntoInterface(m, "approve", data); err != nil {
		return nil, err
	}
...
type ABITransferMethod struct {
	Recipient      common.Address
	Amount         *big.Int
	RawTransaction sdkTransactionTypes.RawTransaction
}

func (_Main *MainFilterer) ParseABITransferMethod(parsedTx sdkTransactionTypes.RawTransaction) (*ABITransferMethod, error) {
	data, err := hexutil.Decode(parsedTx.AdditionalData.Data)
	if err != nil {
		return nil, err
	}
	...
	m := new(ABITransferMethod)
	if err := _Main.UnpackMethodIntoInterface(m, "transfer", data); err != nil {
		return nil, err
	}
	m.RawTransaction = parsedTx
	return m, nil
