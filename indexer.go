...
func NewTransferEventTracker(contractAddress string, orm *gorm.DB, ethClient *ethclient.Client, chainID string) (TransferEventTracker, error) {
	err := orm.AutoMigrate(&TransferEvent{})
	if err != nil {
		return TransferEventTracker{}, err
	}
	filterer, err := NewMainFilterer(common.HexToAddress(contractAddress), ethClient)
...
func (a TransferEventTracker) CreateTransfer(txLog blockchain.ReceiptLog, ethClient *EthBlockChainRPCWithRetry, u *UnmarshalSDKWrapper, txDetailsMap map[string]sdkTransactionTypes.TxnByID) (TransferEvent, error) {
	ethLog := ToEthLog(txLog)
	abiEvent, err := a.MainFilterer.ParseABITransferEvent(ethLog, *txLog.Log)
	if err != nil {
	...
	event := convertToTransferEvent(abiEvent, transactionData, a.chainID)
	return event, nil
}

func EventIndexCallback(orm *gorm.DB, u *UnmarshalSDKWrapper, approvalEventTracker ApprovalEventTracker, transferEventTracker TransferEventTracker, txLogs []ethrpc.Log, ethClient *EthBlockChainRPCWithRetry) error {
	errs, _ := errgroup.WithContext(context.Background())

	approvalEventsLock := new(sync.Mutex)
	approvalEvents := make([]ApprovalEvent, 0)

	transferEventsLock := new(sync.Mutex)
	transferEvents := make([]TransferEvent, 0)

	transactionDetailsMap := getBulkTransactionDetails(txLogs, u, SyncDB{}, false)
	...
	for i := range txLogs {
		...
		errs.Go(func() error {
			return func(i int) error {
				...
				case GetTransferEventHash():
					event, err := transferEventTracker.CreateTransfer(txLog.(blockchain.ReceiptLog), ethClient, u, transactionDetailsMap)
					if err != nil {
						log.WithFields(log.Fields{"txHash": txLog.GetTransactionHash(), "blockNumber": txLog.GetBlockNum(), "err": err}).
							Error("TransferEvent: Failed to create event from log")
						return err
					}
					...
					transferEventsLock.Lock()
					transferEvents = append(transferEvents, event)
					transferEventsLock.Unlock()
	...
	err = orm.Clauses(clause.OnConflict{DoNothing: true}).Create(&transferEvents).Error
	if err != nil {
		return err
	}
	for _, event := range transferEvents {
		wg.Add(1)
		go func(routineEvent TransferEvent, wg *sync.WaitGroup) {
			defer wg.Done()
			err = routineEvent.AfterCreateHook(orm)
			if err != nil {
				log.WithFields(log.Fields{"err": err, "Entity": "Transfer"}).Error("EventIndexCallback: Error making post create hook call")
			}
		}(event, wg)
	}
...
func getBulkTransactionDetails(txLogs []ethrpc.Log, u *UnmarshalSDKWrapper, syncDB SyncDB, isChild bool) map[string]sdkTransactionTypes.TxnByID {
	var transactionDetails []sdkTransactionTypes.TxnByID
	var transactionHashes []string

	for i, txLog := range txLogs {
		if isChild {
			_, isPresent := syncDB.pairSet[strings.ToLower(txLog.Address)]
			if isPresent {
...
type TransferMethodTracker struct {
	Orm          *gorm.DB
	MainFilterer *MainFilterer
	chainID      string
}

func NewTransferMethodTracker(contractAddress string, orm *gorm.DB, ethClient *ethclient.Client, chainID string) (TransferMethodTracker, error) {
	err := orm.AutoMigrate(&TransferMethod{})
	if err != nil {
		return TransferMethodTracker{}, err
	}
	filterer, err := NewMainFilterer(common.HexToAddress(contractAddress), ethClient)
	return TransferMethodTracker{
		Orm:          orm,
		MainFilterer: filterer,
		chainID:      chainID,
	}, err
...
func (a TransferMethodTracker) CreateTransfer(tx sdkTransactionTypes.RawTransaction) (TransferMethod, error) {
	abiMethod, err := a.MainFilterer.ParseABITransferMethod(tx)
	if err != nil {
		return TransferMethod{}, err
	}
	method := convertToTransferMethod(abiMethod, a.chainID)
	return method, err
}

type ApproveMethodTracker struct {
	Orm          *gorm.DB
	MainFilterer *MainFilterer
	chainID      string
}

func NewApproveMethodTracker(contractAddress string, orm *gorm.DB, ethClient *ethclient.Client, chainID string) (ApproveMethodTracker, error) {
	err := orm.AutoMigrate(&ApproveMethod{})
	if err != nil {
		return ApproveMethodTracker{}, err
	}
	filterer, err := NewMainFilterer(common.HexToAddress(contractAddress), ethClient)
	return ApproveMethodTracker{
		Orm:          orm,
		MainFilterer: filterer,
		chainID:      chainID,
	}, err
...
func (a ApproveMethodTracker) CreateApprove(tx sdkTransactionTypes.RawTransaction) (ApproveMethod, error) {
	abiMethod, err := a.MainFilterer.ParseABIApproveMethod(tx)
	if err != nil {
		return ApproveMethod{}, err
	}
	method := convertToApproveMethod(abiMethod, a.chainID)
	return method, err
}

func MethodIndexerCallback(orm *gorm.DB, transferMethodTracker TransferMethodTracker, approveMethodTracker ApproveMethodTracker, from, to int, contractAddress string, unmarshalSDKWrapper *UnmarshalSDKWrapper) error {
	errs, _ := errgroup.WithContext(context.Background())

	transferMethodsLock := new(sync.Mutex)
	transferMethods := make([]TransferMethod, 0)

	approveMethodsLock := new(sync.Mutex)
	approveMethods := make([]ApproveMethod, 0)
	...
	for j := range transactionList {
		...
		errs.Go(func() error {
			return func(j int) error {
				...
				case GetTransferMethodHash():
					method, err := transferMethodTracker.CreateTransfer(tx)
					if err != nil {
						log.WithFields(log.Fields{"txHash": tx.TxHash, "blockNumber": tx.BlockNumber, "err": err}).
							Error("TransferMethod: Failed to write the method execution")
						return err
					}
					...
					transferMethodsLock.Lock()
					transferMethods = append(transferMethods, method)
					transferMethodsLock.Unlock()

				case GetApproveMethodHash():
					method, err := approveMethodTracker.CreateApprove(tx)
					if err != nil {
						log.WithFields(log.Fields{"txHash": tx.TxHash, "blockNumber": tx.BlockNumber, "err": err}).
							Error("ApproveMethod: Failed to write the method execution")
						return err
					}
					err = method.BeforeCreateHook(orm)
					if err != nil {
					...
					approveMethodsLock.Lock()
					approveMethods = append(approveMethods, method)
					approveMethodsLock.Unlock()
	...
	err = orm.Clauses(clause.OnConflict{DoNothing: true}).Create(&transferMethods).Error
	if err != nil {
		return err
	}
	for _, method := range transferMethods {
		wg.Add(1)
		go func(routineMethod TransferMethod, wg *sync.WaitGroup) {
			defer wg.Done()
			err = routineMethod.AfterCreateHook(orm)
			if err != nil {
				log.WithFields(log.Fields{"err": err, "Entity": "Transfer"}).Error("MethodIndexerCallback: Error making post create hook call")
			}
		}(method, wg)
	}

	err = orm.Clauses(clause.OnConflict{DoNothing: true}).Create(&approveMethods).Error
	if err != nil {
		return err
	}
	for _, method := range approveMethods {
		wg.Add(1)
		go func(routineMethod ApproveMethod, wg *sync.WaitGroup) {
			defer wg.Done()
			err = routineMethod.AfterCreateHook(orm)
			if err != nil {
				log.WithFields(log.Fields{"err": err, "Entity": "Approve"}).Error("MethodIndexerCallback: Error making post create hook call")
			}
		}(method, wg)
	}
...
type SyncDB struct {
	orm     *gorm.DB
	pairSet map[string]map[string]interface{}
...
func RunIndexer(config IndexerConfig) {
	...
	topicsInterestedIn := []string{
		...
		GetTransferEventHash(),
	}
	methodsInterestedIn := []string{
		GetTransferMethodHash(),
		GetApproveMethodHash(),
	}
...
func processLogsFromChannel(orm *gorm.DB, syncDB SyncDB, parallelCallForDB int, chainID string, config IndexerConfig, ethClient *ethclient.Client, receiptsChannel chan Receipts, ethRPCClient *EthBlockChainRPCWithRetry, unmarshalSDKWrapper *UnmarshalSDKWrapper, wg *sync.WaitGroup) {
	...
	transferEventTracker, err := NewTransferEventTracker(config.ContractAddress, orm, ethClient, chainID)
	if err != nil {
		panic(err)
	}
		...
		for true {
			...
			for i := 0; i < parallelCallForDB; i++ {
				...
				go func(routineReceiptLog Receipts) {
					...
					err := EventIndexCallback(orm, unmarshalSDKWrapper, approvalEventTracker, transferEventTracker, routineReceiptLog.receipts, ethRPCClient)
					if err != nil {
...
func syncMethods(orm *gorm.DB, syncDB SyncDB, rpcClient *EthBlockChainRPCWithRetry, config IndexerConfig, ethClient *ethclient.Client, chainID string, unmarshalSDKWrapper *UnmarshalSDKWrapper) {
	...
	transferMethodTracker, err := NewTransferMethodTracker(config.ContractAddress, orm, ethClient, chainID)
	if err != nil {
		log.WithField("Tracker Name", "Transfer").Error("Failed to create method Tracker")
		panic(err)
	}

	approveMethodTracker, err := NewApproveMethodTracker(config.ContractAddress, orm, ethClient, chainID)
	if err != nil {
		log.WithField("Tracker Name", "Approve").Error("Failed to create method Tracker")
		panic(err)
	}
		...
		for true {
			...
			for i := 0; i < parallelCalls; i++ {
				...
				go func(routineFrom, routineTo int) {
					...
					err = MethodIndexerCallback(orm, transferMethodTracker, approveMethodTracker, routineFrom, routineTo, config.ContractAddress, unmarshalSDKWrapper)
					if err != nil {
